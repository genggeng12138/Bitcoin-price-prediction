...
class PSO:
    ...
    def fitness(self, ind_var):
        ...
        clf = TwinSVMRegressor(C1=x1, C2=x1, kernel_type=3, kernel_param=x2)
        clf.fit(X, y)
        predictval = clf.predict(x_test)
        print("R2 = ", explained_variance_score(y_test, predictval))  # R2
        return explained_variance_score(y_test, predictval)
...
if __name__ == '__main__':
    ...
    pso = PSO(parameters)
    pso_lssvr = pso.main()
    pso_lssvr = pd.DataFrame(pso_lssvr)
    pso_lssvr.to_excel(r"C:\Users\mjgeng\Desktop\比特币及其相关数据\psotwsvr27.xlsx")